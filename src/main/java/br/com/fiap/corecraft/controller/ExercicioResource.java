...
import br.com.fiap.corecraft.model.repository.ExercicioRepository;
...
public class ExercicioResource {
	...
	public Response findAll() {
		...
		List<Exercicio> retorno = ExercicioRepository.findAll();
...
//
//		final URI userUri = UriBuilder.fromResource(ExercicioResource.class).path("/user/{id}").build(resp.getId());
//
...
//
//			final URI userUri = UriBuilder.fromResource(ExercicioResource.class).path("/user/{id}").build(novo.getId());
//
//			ResponseBuilder response = Response.created(userUri);
...
//	}
	...
	@GET
	@Path("/{id}")
	public Response findById(@PathParam("id") Long exercicioId) {
		Exercicio exercicio = ExercicioRepository.findById(exercicioId);
		if (exercicio != null) {
			ResponseBuilder response = Response.ok();
			response.entity(exercicio);
			return response.build();
